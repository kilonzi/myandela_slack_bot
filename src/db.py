...
from .config import MONGO_USERNAME, MONGO_PASSWORD, MONGO_DB
import pymongo
...
def parse_submission(data):
    ...
    body = {
        ...
        'good_stuff': get_select_options(state['block_good_stuff']['text_good_stuff']['selected_options']),
        'frustrations': get_select_options(state['block_frustrations']['text_frustrations']['selected_options']),
        'recent_accomplishments': state['block_recent_accomplishments']['recent_accomplishments']['value'],
...
def save_check_in(data):
    ...
    return db.checks.insert_one(body)
